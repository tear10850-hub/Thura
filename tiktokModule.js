...
// Render Server ပေါ်တွင် Fetch Error မတက်အောင် Header ပါဝင်သော API
async function fetchTikTokData(url) {
  ...
  // 1st API Endpoint (Loverslab API)
  try {
    const apiRes = await fetch(`https://api.tiklydown.eu.org/api/download?url=${encodeURIComponent(url)}`, { headers });
    if (apiRes.ok) {
  ...
  // 2nd Backup API Endpoint (TikWM POST Method)
  try {
    ...
    const backupRes = await fetch('https://www.tikwm.com/api/', {
      method: 'POST',
      headers: {
    ...
    if (backupRes.ok) {
      ...
      if (backupData && backupData.code === 0 && backupData.data) {
        return {
    ...
    console.error("All TikTok APIs Failed:", error.message);
  }
