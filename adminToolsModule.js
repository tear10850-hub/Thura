...
// Premium ခွင့်ပြုချက် စစ်ဆေးခြင်း
// 💡 စည်းကမ်း- 1. Owner ဖြစ်ရမည် (သို့မဟုတ်) 2. Key ဝယ်ထားသူ ဖြစ်သည့်အပြင် ယခု Group ၏ Admin လည်း ဖြစ်ရမည်။
async function hasVideoPermission(ctx) {
  if (ctx.from.id === OWNER_ID) return true;
  
  // Admin မဟုတ်ပါက ငြင်းပယ်မည်
...
နှင်ထုပ်သူAdmin😉 - ${operatorText}`;
      ...
      await ctx.reply("❌ မန်ဘာအား Ban ရာတွင် အဆင်မပြေပါ (Bot တွင် Ban Permission ရှိမရှိ စစ်ဆေးပါ)");
    }
  });
  ...
  bot.command('Mute', async (ctx) => {
    ...
    try {
      ...
      const captionText = 
`🚨--Mute--🚨
ရှာရှည်လွန်းသဖြင့်😂အမြင်ကပ်ပုဏ်မဖြင့်
...
Umutrလွှတ်ပေးသူ - ${operatorText}🤓`;
  ...
  // ====================================================
  // 2. VIDEO CUSTOMIZATION COMMANDS (PREMIUM + ADMIN CHECK)
  // ====================================================

  // BAN VIDEO
  bot.command('bvideo', async (ctx) => {
    if (!(await hasVideoPermission(ctx))) {
      return ctx.reply("❌ ဤ စနစ်ကို အသုံးပြုရန် Premium Key လိုအပ်သည့်အပြင် Group Admin သို့မဟုတ် Owner ဖြစ်ရပါမည်။");
    }
    const replyMsg = ctx.message.reply_to_message;
