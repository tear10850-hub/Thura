...
📝 **Bio:** ${botBio}`;
    ...
    const mediaId = botProfile?.botPfMediaId;
    const mediaType = botProfile?.botPfMediaType; // 'video' or 'photo'
    ...
    if (mediaId && mediaType === 'video') {
      try {
        await ctx.replyWithVideo(mediaId, { caption, parse_mode: 'Markdown', reply_markup: keyboard });
        return;
      } catch (e) {}
    } else if (mediaId && mediaType === 'photo') {
      try {
        await ctx.replyWithPhoto(mediaId, { caption, parse_mode: 'Markdown', reply_markup: keyboard });
        return;
      } catch (e) {}
    }
...
🔕 **ပိတ်ခိုင်းသူ** - ${getUserMention(ctx.from)}`;
  ...
  bot.command(['umute', 'Umute', 'UMute'], async (ctx) => {
    ...
    try {
      ...
      const captionText = 
`🚨-- 𝑼𝑵𝑴𝑼𝑻𝑬 𝑺𝒀𝑺𝑻𝑬𝑴 --🚨
မျက်နှာချိုပြီး ချွဲလို့ စာပို့ခွင့် ပြန်လွှတ်ပေးလိုက်ပါပြီရှင် 🙂😜✨
...
🎉 **လွှတ်ပေးသူ** - ${getUserMention(ctx.from)} 🤓`;
  ...
  // Video Setter Commands (with check if already exists)
