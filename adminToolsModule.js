...
async function renderAdminProfilePage(ctx, pageIndex, isEdit = false) {
  ...
  // A. နောက်ဆုံး စာမျက်နှာ (BOT PROFILE PAGE)
...
📝 Bio: ${bio}`;
  ...
  // Telegram Profile Picture ယူခြင်း
  let photoId = null;
  try {
...
Umutrလွှတ်ပေးသူ - ${getUserMention(ctx.from)}🤓`;
  ...
  bot.command('mvideo', async (ctx) => {
    if (!(await hasVideoPermission(ctx))) return ctx.reply("❌ Premium Key လိုအပ်ပါသည်။");
    const replyMsg = ctx.message.reply_to_message;
    if (!replyMsg || !replyMsg.video) return ctx.reply("ကျေးဇူးပြု၍ Video ကို Reply ထောက်ပြီး /mvideo`bvideo`, `mvideo`, `uvideo` command များ၏ syntax များကိုလည်း Fix လုပ်ပေးထားပါသည်။

အထက်ပါ Code အသစ်ဖြင့် လဲလှယ်လိုက်ပါက အောက်ပါအတိုင်း အတိအကျ အလုပ်လုပ်ဆောင်သွားမည်ဖြစ်သည်-

1. `/adminpf` ဟု ရိုက်လိုက်လျှင် Admin များ၏ **Telegram Profile Picture** (ရှိလျှင်) နှင့် အချက်အလက်များကို စာမျက်နှာတစ်မျက်နှာချင်းစီပြပါမည်။
2. Inline Button **"နောက်သို့➡️"** နှင့် **"ရှေ့သို့⬅️"** ကို နှိပ်၍ Admin တစ်ဦးချင်းစီသို့ ပြောင်းကြည့်နိုင်မည်။
3. **နောက်ဆုံး စာမျက်နှာ (Bot ၏ စာမျက်နှာ)** သို့ ရောက်သွားချိန်တွင်မှ **Bot Custom Video** ပြသပေးပြီး `/setbotname`, `/setbotid`, `/setbotusername`, `/setbotbio` တို့တွင် မှတ်ထားခဲ့သော Bot Profile အချက်အလက်များကို ပြသပေးမည်ဖြစ်ပါသည်။
