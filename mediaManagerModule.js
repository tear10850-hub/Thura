...
function setupMediaManagerModule(bot, OWNER_ID) {
  ...
  bot.command('showstickers', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return;
    const targetChannel = ctx.match ? ctx.match.trim() : null;
    if (!targetChannel) return ctx.reply("⚠️ Channel Username သို့မဟုတ် ID ထည့်ပေးပါ။ (ဥပမာ - `/showstickers @mychannel`)", { parse_mode: 'Markdown' });
    ...
    try {
      ...
      for (const stk of stickers) {
        try {
          ...
          console.error(`Error sending sticker ${index}:`, e.message);
        }
      }
      await ctx.reply(`✅ Sticker များ အားလုံးကို ${targetChannel} သို့ ပို့ဆောင်ပြီးပါပြီ။`);
    } catch (err) {
  ...
  bot.command('showphotos', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return;
    const targetChannel = ctx.match ? ctx.match.trim() : null;
    if (!targetChannel) return ctx.reply("⚠️ Channel Username သို့မဟုတ် ID ထည့်ပေးပါ။ (ဥပမာ - `/showphotos @mychannel`)", { parse_mode: 'Markdown' });
    ...
    try {
      ...
      for (const item of photos) {
        try {
          ...
          console.error(`Error sending photo ${index}:`, e.message);
        }
      }
      await ctx.reply(`✅ ဓာတ်ပုံများ အားလုံးကို ${targetChannel} သို့ ပို့ဆောင်ပြီးပါပြီ။`);
    } catch (err) {
  ...
  bot.command('showvideos', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return;
    const targetChannel = ctx.match ? ctx.match.trim() : null;
    if (!targetChannel) return ctx.reply("⚠️ Channel Username သို့မဟုတ် ID ထည့်ပေးပါ။ (ဥပမာ - `/showvideos @mychannel`)", { parse_mode: 'Markdown' });
    ...
    try {
      ...
      for (const item of videos) {
        try {
          ...
          console.error(`Error sending video ${index}:`, e.message);
        }
      }
      await ctx.reply(`✅ ဗီဒီယိုများ အားလုံးကို ${targetChannel} သို့ ပို့ဆောင်ပြီးပါပြီ။`);
    } catch (err) {
  ...
  bot.command('showaudios', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return;
    const targetChannel = ctx.match ? ctx.match.trim() : null;
    if (!targetChannel) return ctx.reply("⚠️ Channel Username သို့မဟုတ် ID ထည့်ပေးပါ။ (ဥပမာ - `/showaudios @mychannel`)", { parse_mode: 'Markdown' });
    ...
    try {
      ...
      for (const item of audios) {
        try {
          ...
          console.error(`Error sending audio ${index}:`, e.message);
        }
      }
      await ctx.reply(`✅ Audio များ အားလုံးကို ${targetChannel} သို့ ပို့ဆောင်ပြီးပါပြီ။`);
    } catch (err) {
