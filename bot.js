...
// ============================================================
// ⏰ အချိန်ထိန်းချုပ်မှု Commands (/rtime, /ntime, /drtime, /artime, /antime, /adrtime)
// ============================================================
bot.command('rtime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  const timeArg = ctx.message.text.replace('/rtime', '').trim();
  if (!timeArg) {
    const sent = await ctx.reply('⚠️ အချိန်ထည့်ပေးပါရှင်။ (ဥပမာ: /rtime 1:00 AM)');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  let setting = await BotSetting.findOne({ chatId: ctx.chat.id });
  if (setting && setting.rtime) {
    const sent = await ctx.reply('⚠️ အချိန်သတ်မှတ်ပြီးသား ရှိနေပါသည်! အသစ်ပြောင်းချင်ပါက ပထမဆုံး /drtime ဖြင့် အရင်ဖျက်ပေးပါ။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { rtime: timeArg }, { upsert: true, new: true });
  const sent = await ctx.reply(`🔴 ဤအချိန် (${timeArg}) မှစ၍ Bot စာမပြန်တော့ပါ။`);
  global.autoDeleteMessage(ctx, sent.message_id);
});

bot.command('ntime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  const timeArg = ctx.message.text.replace('/ntime', '').trim();
  if (!timeArg) {
    const sent = await ctx.reply('⚠️ အချိန်ထည့်ပေးပါရှင်။ (ဥပမာ: /ntime 24:00 PM)');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { ntime: timeArg }, { upsert: true, new: true });
  const sent = await ctx.reply(`🟢 ဤအချိန် (${timeArg}) ကျမှ Bot ပြန်လည် စကားပြောပါမည်။`);
  global.autoDeleteMessage(ctx, sent.message_id);
});

bot.command('drtime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { rtime: null, ntime: null });
  const sent = await ctx.reply('🗑️ ယာယီ သတ်မှတ်ထားသော အချိန်များကို ဖျက်လိုက်ပါပြီ။');
  global.autoDeleteMessage(ctx, sent.message_id);
});

bot.command('artime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  const timeArg = ctx.message.text.replace('/artime', '').trim();
  if (!timeArg) {
    const sent = await ctx.reply('⚠️ အချိန်ထည့်ပေးပါရှင်။ (ဥပမာ: /artime 01:00 AM)');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  let setting = await BotSetting.findOne({ chatId: ctx.chat.id });
  if (setting && setting.artime) {
    const sent = await ctx.reply('⚠️ နေ့စဉ်အချိန် သတ်မှတ်ပြီးသား ရှိနေပါသည်! အသစ်ပြောင်းချင်ပါက /adrtime ဖြင့် အရင်ဖျက်ပေးပါ။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { artime: timeArg }, { upsert: true, new: true });
  const sent = await ctx.reply(`📅 နေ့စဉ် ဤအချိန် (${timeArg}) တိုင်း Bot စာမပြန်တော့ပါ။`);
  global.autoDeleteMessage(ctx, sent.message_id);
});

bot.command('antime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  const timeArg = ctx.message.text.replace('/antime', '').trim();
  if (!timeArg) {
    const sent = await ctx.reply('⚠️ အချိန်ထည့်ပေးပါရှင်။ (ဥပမာ: /antime 24:00 PM)');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }

  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { antime: timeArg }, { upsert: true, new: true });
  const sent = await ctx.reply(`📅 နေ့စဉ် ဤအချိန် (${timeArg}) တိုင်း Bot ပြန်လည် စကားပြောပါမည်။`);
  global.autoDeleteMessage(ctx, sent.message_id);
});

bot.command('adrtime', async (ctx) => {
  if (!await isAdminOrOwner(ctx)) {
    const sent = await ctx.reply('⛔ Group Admin သို့မဟုတ် Owner သာ အသုံးပြုခွင့်ရှိပါသည်။');
    return global.autoDeleteMessage(ctx, sent.message_id);
  }
  await BotSetting.findOneAndUpdate({ chatId: ctx.chat.id }, { artime: null, antime: null });
  const sent = await ctx.reply('🗑️ နေ့စဉ် သတ်မှတ်ထားသော အချိန်များကို ဖျက်လိုက်ပါပြီ။');
  global.autoDeleteMessage(ctx, sent.message_id);
});
...
bot.on('message', async (ctx, next) => {
  ...
  try {
    // ⏰ အချိန်သတ်မှတ်ချက်အရ စာမပြန်ရန် ပိတ်ထားခြင်း ရှိမရှိ စစ်ဆေးခြင်း
...
console.log('🤖 Bot is running smoothly with Reactions, Time Controls & Channel Support!');
