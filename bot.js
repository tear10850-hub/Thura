...
// အသစ်ထည့်သွင်းမည့် ဖိုင်နှစ်ခုနှင့် scriptModule
const setupChannelGroupModule = require('./channelGroupManager');
const setupBotLeaveModule = require('./botLeaveModule');
const { setupScriptModule } = require('./scriptModule'); // 🆕 scriptModule ကို ချိတ်ဆက်ခြင်း
...
// ဖိုင်များကို bot သို့ ချိတ်ဆက်ခြင်း
setupChannelGroupModule(bot, OWNER_ID);
setupBotLeaveModule(bot, OWNER_ID);
setupScriptModule(bot, OWNER_ID); // 🆕 scriptModule ကို ထည့်သွင်းချိတ်ဆက်ပေးထားပါပြီ
    ...
    if (msg.text && !urlRegex.test(msg.text)) {
      ...
      if (!isCommand) {
        try {
          ...
          console.error('AI Handler Error:', aiErr.message);
        }
