...
// ============================================================
// ⭐ Callback Query Handler (ခလုပ်တွေအတွက် - အရေးကြီးဆုံး)
// ============================================================
bot.on('callback_query:data', async (ctx) => {
  try {
    ...
    await ctx.answerCallbackQuery({ text: '✅ ခလုပ်နိပ်ပြီး!' });
    await ctx.editMessageText('🔄 ခလုပ်နိပ်ပြီးပါပြီ။');
  } catch (error) {
...
bot.on('message', async (ctx, next) => {
  // ✅ ခလုပ်တွေကို မပိတ်မိအောင်
  if (ctx.callbackQuery) {
    return next();
  }

  // ✅ Group တွေမှာ စကားပြောမရရင် Error မတက်အောင်
  try {
    // Group ဖြစ်ပြီး စကားပြောမရဘူးဆိုရင် ဘာမှမလုပ်ဘူး
    if (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup') {
      // Group မှာ စကားပြောခွင့်မရှိရင် ဒီမှာထွက်
      // (ဒါက ခင်ဗျား သတ်မှတ်ချင်တဲ့ Group တွေအတွက်)
      // ဥပမာ - သတ်မှတ်ထားတဲ့ Group ID များ
      const allowedGroups = []; // ခွင့်ပြုထားတဲ့ Group ID တွေထည့်
      if (allowedGroups.length > 0 && !allowedGroups.includes(ctx.chat.id.toString())) {
        return; // ခွင့်မပြုထားတဲ့ Group ဆိုရင် ဘာမှမလုပ်ဘူး
      }
    }
  } catch (err) {
  ...
  try {
    ...
    // Error ဖြစ်ရင်လည်း Bot မပျက်အောင်
  }
});
