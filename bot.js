...
// ============================================================
// ⏰ Auto Delete Bot Messages (၃ မိနစ်ပြည့်တိုင်း အကုန်ဖျက်မယ်)
// ============================================================
const botMessages = new Map();

// Bot ပို့တဲ့ Message အကုန်ကို မှတ်ထားမယ်
...
// ၃ မိနစ်ပြည့်တိုင်း Bot ပို့တဲ့စာတွေကိုဖျက်မယ်
async function autoDeleteBotMessages(bot, chatId, messageId) {
  setTimeout(async () => {
    try {
      await bot.api.deleteMessage(chatId, messageId);
      console.log(`🗑️ Auto deleted message ${messageId} in ${chatId}`);
      if (botMessages.has(chatId)) {
        const msgs = botMessages.get(chatId);
        botMessages.set(chatId, msgs.filter(id => id !== messageId));
      }
    } catch (err) {}
  }, 3 * 60 * 1000); // ၃ မိနစ်
}
...
// ============================================================
// 📌 Announcement Messages (မဖျက်ရတဲ့စာတွေ)
// ============================================================
const ANNOUNCEMENT_MESSAGES = new Set();

// Announcement သတ်မှတ်ရန်
bot.command('setannounce', async (ctx) => {
  if (ctx.from.id !== OWNER_ID) {
    return ctx.reply('⛔ Owner သာသုံးလို့ရပါတယ်။');
  }
  
  if (!ctx.message.reply_to_message) {
    return ctx.reply('⚠️ Announcement လုပ်ချင်တဲ့ Message ကို reply ထောက်ပြီး /setannounce လုပ်ပါ။');
  }
  
  const msgId = ctx.message.reply_to_message.message_id;
  ANNOUNCEMENT_MESSAGES.add(msgId);
  await ctx.reply(`✅ Message ${msgId} ကို Announcement အဖြစ်သတ်မှတ်ပြီး မဖျက်တော့ပါ။`);
});

// Announcement ဖယ်ရှားရန်
bot.command('removeannounce', async (ctx) => {
  if (ctx.from.id !== OWNER_ID) {
    return ctx.reply('⛔ Owner သာသုံးလို့ရပါတယ်။');
  }
  
  if (!ctx.message.reply_to_message) {
    return ctx.reply('⚠️ Announcement ဖယ်ချင်တဲ့ Message ကို reply ထောက်ပြီး /removeannounce လုပ်ပါ။');
  }
  
  const msgId = ctx.message.reply_to_message.message_id;
  ANNOUNCEMENT_MESSAGES.delete(msgId);
  await ctx.reply(`✅ Message ${msgId} ကို Announcement စာရင်းကနေ ဖယ်လိုက်ပါပြီ။`);
});

// ============================================================
// Global Auto Delete Function (Announcement မပါအောင်)
// ============================================================
global.autoDeleteMessage = function(ctx, messageId, delayTime = 3 * 60 * 1000) {
  // Announcement ဆိုရင် မဖျက်ဘူး
  if (ANNOUNCEMENT_MESSAGES.has(messageId)) {
    console.log(`📌 Announcement message ${messageId} - မဖျက်ပါ။`);
    return;
  }
  
  // Bot ပို့တဲ့စာကို မှတ်ထား
  trackBotMessage(ctx.chat.id, messageId);
