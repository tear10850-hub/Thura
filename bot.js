...
bot.on('message', async (ctx, next) => {
  ...
  await next(); // နောက် middleware များကို ဆက်လက် အလုပ်လုပ်စေသည်
});
  ...
  try {
    ...
    if (isOwner) {
      ...
      if (type && fileId) {
        ...
        return; // Owner က မီဒီယာသိမ်းသည့်အခါတွင်သာ ဒီနေရာမှာတင် ရပ်မည်
      }
    }
  } catch (error) {
...
// ----------------------------------------------------
// ၆. မန်ဘာများ မီဒီယာပို့ပါက Database မှ Random ပြန်ပို့ပေးခြင်း
// ----------------------------------------------------
bot.on('message', async (ctx, next) => {
  try {
    const msg = ctx.message;

    // Command စာများ သို့မဟုတ် သာမန် စာအုပ်များဖြစ်ပါက အောက်သို့ ဆက်သွားပါမည်
    ...
    if (requestedType) {
      ...
      if (mediaCount > 0) {
        ...
        if (randomMedia) {
          ...
          return; // မီဒီယာ ပြန်ပို့ပြီးပါက ဒီနေရာတွင် ရပ်ပါမည်
        }
      }
    }
  } catch (error) {
  ...
  await next(); // မီဒီယာ မဟုတ်ပါက Modules ဆီသို့ ဆက်သွားပါမည်
});

// ----------------------------------------------------
// ၇. Main Bot Commands နှင့် Modules များ ချိတ်ဆက်ခြင်း
// ----------------------------------------------------
...
// Main Bot Modules ကို Register လုပ်ပေးခြင်း
registerMainBotFunctions(bot);

// Clone Module
setupCloneModule(bot, OWNER_ID, SECOND_MONGO_URI);
