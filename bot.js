...
// ----------------------------------------------------
// 🟢 မက်ဆေ့ခ်ျတစ်စောင်စီကို ၃ မိနစ်ပြည့်တိုင်း အလိုအလျောက်ဖျက်ပေးသည့် Function
// ----------------------------------------------------
function autoDeleteMessage(ctx, messageId) {
  setTimeout(async () => {
    try {
...
// Commands: /open, /close, /Tearopen နှင့် /Tear
bot.command('open', async (ctx) => {
  ...
  autoDeleteMessage(ctx, sent.message_id);
});
...
bot.on('message', async (ctx) => {
  try {
    ...
    if (msg.text && !urlRegex.test(msg.text)) {
      ...
      if (!isCommand) {
        // /Tear Mode ဖြစ်နေလျှင် စာမပြန်သေးဘဲ လူ ၃ ယောက် စကားပြောသည်ထိ စောင့်မည်
        if (!isTearOpen) {
        ...
        if (count > 0) {
          ...
          if (randomDoc) {
            ...
            // Reply ထောက်ပြန်မည် + ၃ မိနစ်ကြာလျှင် Auto ဖျက်မည်
            const sentMsg = await ctx.reply(randomDoc.text, replyOptions);
            autoDeleteMessage(ctx, sentMsg.message_id);
            ...
            if (replyCounter >= 8) {
              ...
              if (stickerCount > 0) {
                ...
                if (randomSticker) {
                  ...
                  autoDeleteMessage(ctx, sentSticker.message_id);
                }
              }
            }
          }
    ...
    if (requestedType) {
      ...
      if (mediaCount > 0) {
        ...
        if (randomMedia) {
          ...
          if (sentMedia) {
            autoDeleteMessage(ctx, sentMedia.message_id);
          }
        }
      }
    }
...
// Auto Clean Up (၃၀ ရက်ကျော် စာများကို ဖျက်ခြင်း)
setInterval(async () => {
  try {
