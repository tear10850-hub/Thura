...
function setupMediaFeed(bot) {
  ...
  bot.command('public', async (ctx) => {
    ...
    if (!latestVideo) return ctx.reply('⚠️ ဗီဒီယို မရှိသေးပါ။');

    latestVideo.visibility = 'public';
    await latestVideo.save();
    await ctx.reply('✅ ဗီဒီယိုကို **အများသုံး (Public)** သို့ ပြောင်းပြီးပါပြီ။');
  });
  ...
  bot.command('friends_only', async (ctx) => {
    ...
    await latestVideo.save();
    await ctx.reply('✅ ဗီဒီယိုကို **သူငယ်ချင်းများသာ** သို့ ပြောင်းပြီးပါပြီ။');
  });

  // 💬 Comment စာသား လက်ခံခြင်း
  bot.on('message:text', async (ctx, next) => {
    ...
    if (commentData) {
      ...
      await Video.updateOne(
        { videoId },
        { $push: { comments: { chatId, name, text: commentText } } }
      );
