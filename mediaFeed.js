...
function setupMediaFeed(bot) {
  ...
  bot.on('message:video', async (ctx) => {
    ...
    const newVideo = new Video({
      videoId: videoId,
      chatId: chatId,  // String အနေနဲ့သိမ်း
  ...
  bot.on('callback_query:data', async (ctx) => {
    ...
    if (data.startsWith('like_')) {
      ...
      if (!vid) {
        return ctx.answerCallbackQuery({ text: '⚠️ ဗီဒီယို မတွေ့ပါ။', show_alert: true });
      }

      // Like လုပ်ခဲ့ပြီးရင် ဖြုတ်၊ မလုပ်ရင် ထည့်
    ...
    if (data.startsWith('delete_')) {
      ...
      // ChatId ကို String ချင်းနှိုင်း
    ...
    // ❌ မသိသော ခလုပ်
    await ctx.answerCallbackQuery({ text: '⚠️ ဤခလုပ်ကို မသိပါ။' });
  });
