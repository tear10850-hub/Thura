...
async function handleAIResponse(ctx, userText, replyOptions, OWNER_ID) {
  ...
  try {
    ...
    if (!userText || !userText.startsWith('/ai')) return false;
    ...
    // SDK ဗားရှင်းအမျိုးမျိုးအတွက် response text ကို လုံခြုံစွာ ထုတ်ယူခြင်း
    let aiReplyText = "";
    if (response) {
      if (typeof response.text === 'function') {
        aiReplyText = await response.text();
      } else if (typeof response.text === 'string') {
        aiReplyText = response.text;
      } else if (response.candidates && response.candidates[0]?.content?.parts[0]?.text) {
        aiReplyText = response.candidates[0].content.parts[0].text;
      }
    }

    if (aiReplyText) {
      const textReply = cleanTextForTG(aiReplyText);
      ...
      throw new Error("AI မှ စာသားတုံ့ပြန်မှု မရရှိပါရှင့်။");
    }
